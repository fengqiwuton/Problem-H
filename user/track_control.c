...
#define TRACK_MAX_TURN       60
#define TRACK_MIN_EDGE_TURN  245
#define TRACK_KP_DEFAULT     80
#define TRACK_KD_DEFAULT     0
#define STRAIGHT_SPEED       200
#define ANGLE_SPEED          160
#define CURVE_ENTER_ERR       65
#define CURVE_EXIT_ERR        35
#define TRACK_TRIM           10   /* 左前轮偏补偿: 正值=右轮加速 */
...
void track_car_drive(int left_speed, int right_speed)
{
	...
	control_speed(-drive_right_now - TRACK_TRIM, -drive_right_now - TRACK_TRIM, -drive_left_now + TRACK_TRIM, -drive_left_now + TRACK_TRIM);
}
...
static int calc_track_error(uint8_t bits)
{
	 ...
	 */
	static const int sensor_weight[8] = {-170, -130, -70, -30, 30, 70, 130, 170};
	int sum = 0;
	uint8_t i;
	uint8_t left_count;
	uint8_t right_count;
...
void track_follow_update(void)
{
	...
	if(sensor_active_count == 0)
	{
		...
		track_car_drive(TRACK_RECOVERY_FORWARD, TRACK_RECOVERY_FORWARD);
		return;
	}
...

	{
		...
		{
			int spd = TRACK_BASE_SPEED;
			base_speed = spd - abs_error / 8;
			base_speed = limit_int(base_speed, TRACK_EDGE_SPEED, STRAIGHT_SPEED);
		}
	}
