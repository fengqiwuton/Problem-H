...
### 历史/已停用方案：八路循迹模块（GPIO 直连）

用途：历史版本通过 8 路数字量输入读取黑线位置并做巡线闭环。
...
- 本 GPIO 直连方案已停用。当前循迹仅以后文的“当前最终接线”和“当前连续循迹版本”为准：使用 UART1 的 PA9/PA10，PB10/PB11 不接循迹模块。

### 历史/已停用方案：八路循迹模块（UART3 / PB10、PB11）

用途：历史版本通过 UART3 接收官方协议的 8 路循迹数字量数据，并解析 `$D,x1:0,x2:0,...,x8:0#`。
